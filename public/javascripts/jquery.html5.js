...
var form=$(this);var required=new Array();var email=new Array();function fillInput(input){input.val(input.attr('placeholder'));input.css('color',opts.colorOff);}
if(opts.labels=='hide'){$(this).find('label').hide();}
$.each($('select',this),function(){$(this).css('color',opts.colorOff);$(this).change(function(){$(this).css('color',opts.colorOn);});});$.each($(':input:visible:not(:button, :submit, :radio, :checkbox, select)',form),function(i){fillInput($(this));if($.browser.webkit||$.browser.opera){if($(this).attr('required')){required[i]=$(this);}}
else{if($(this).attr('required')==''){required[i]=$(this);}}
